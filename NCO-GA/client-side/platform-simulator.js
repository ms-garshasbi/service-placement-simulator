...
class serviceGenerator {
    ...
    getReliabilityScore(min, max) {
        ...
        if (rl <= 0.7)
        {
            return getRandomValue(0.989, 0.998);
        }
        else
        {
    ...
    generate()
    {
            ...
            const platformComputation = this.platformComputationalCapacity * (this.capacityPercentage - 0.1);
            const platformMemory = this.platformMemoryCapacity * (this.capacityPercentage - 0.1);
            const platformDisk = this.platformDiskCapacity * 0.2; //Disk is not an important problem.
...
class commands {
    ...
    runAlgorithms ()
    {
        ...
        else if (configurations['algo'] == "NCOtest")
        {
            sendAxiosPost(url, {
                computingNodes: computingNodes,
                helperNodes: helperNodes,
