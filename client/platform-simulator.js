...
function saveAll(resData) {
    ...
    for (
        ...
        if (!algoData) {
            throw new Error(
                `Server returned no result for ${algoName}.`
            );
        }
        ...
        if (!result) {
            throw new Error(
                `${algoName}_result is missing from server response.`
            );
        }

        if (runtime === undefined) {
            throw new Error(
                `${algoName}_runtime is missing from server response.`
            );
        }

        if (!perService) {
            throw new Error(
                `${algoName}_perService_result is missing from server response.`
            );
        }

        const mappedPerService =
            convertMultilineToServiceMap(
                perService
            );
...
class commands {
    ...
    tuningCommand() {
        ...
        if (
            ...
            throw new Error(
                'Tuning mode requires exactly one algorithm.'
            );
        }
        ...
        if (
            ...
            throw new Error(
                `Algorithm ${selected} does not require parameter tuning.`
            );
        }
        ...
        if (
            ...
            throw new Error(
                'The legacy average-scale tuning mode is not supported by this simulator.'
            );
        }
        ...
        if (!grid) {
            throw new Error(
                `No gridTuning configuration was found for ${selected}.`
            );
        }
        ...
        return sendAxiosPost(
            url,
            {
                characteristics: {
                    ...
                    componentConnections:
                        system.componentConnections,

                    infraConnections:
                        system.infraConnections
                },
...
(async () => {
    ...
    if (
        ...
        throw new Error(
            `Unknown configuration type: ${configurations['type']}. Allowed types are: new, current, tuning, optConfig.`
        );
    }
})()
.catch(
